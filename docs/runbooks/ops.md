...
| `skipped-no-remote` | One of five reasons: `no-checkout` (no worktree), `not-a-repo`, `no-origin`, `unparseable-remote`, or `non-github-host`. | Fix the checkout's remote if tracking is wanted; otherwise expected for a project with no GitHub remote. |
