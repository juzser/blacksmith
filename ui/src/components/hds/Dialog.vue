...
<template>
  <Teleport to="body">
    <div v-if="open" class="hds-dialog-overlay" @click="emit('close')">
      <div
        ref="dialogEl"
        class="hds-dialog"
        :class="{ 'hds-dialog--sm': size === 'sm' }"
        :role="role"
        aria-modal="true"
        :aria-label="title"
        ...
        @click.stop
      >
        <div class="hds-dialog__head">
          <div class="hds-dialog__title">{{ title }}</div>
          <button
            type="button"
        ...
        <div class="hds-dialog__body">
          <p v-if="description" :id="descId" class="hds-card__desc">{{ description }}</p>
          <slot />
        </div>
        <div v-if="$slots.footer" class="hds-dialog__footer">
          <slot name="footer" />
        </div>
      </div>
    </div>
