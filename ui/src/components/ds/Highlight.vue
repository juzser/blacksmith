...
<template>
  <div class="ds-hl" :style="style">
    <div class="ds-hl__body">
      <span v-if="eyebrow" class="ds-hl__eyebrow">{{ eyebrow }}</span>
      <h3 v-if="title" class="ds-hl__title">{{ title }}</h3>
      <p v-if="$slots.default" class="ds-hl__text"><slot /></p>
      <div v-if="$slots.action" class="ds-hl__actions"><slot name="action" /></div>
    </div>
  </div>
</template>
