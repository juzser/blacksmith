...
<template>
  <nav class="ds-side" :data-collapsed="collapsed" aria-label="Primary">
    <div class="ds-side__head">
      ...
      <span class="ds-side__mark">
        <img :src="markSrc" :alt="brand" width="24" height="24" decoding="async" />
      </span>
      <span v-if="!collapsed" class="ds-side__word">{{ brand }}</span>
    </div>
    <ul class="ds-side__group">
      <template v-for="(it, i) in items" :key="it.category ? `cat-${it.category}` : it.id ?? i">
        <li v-if="it.category" class="ds-side__cat">{{ it.category }}</li>
        <li v-else>
          <Tooltip v-if="it.disabled" :label="it.disabledReason ?? 'Phase 6b'" side="right">
            <button type="button" class="ds-side__item" disabled :aria-label="it.label">
