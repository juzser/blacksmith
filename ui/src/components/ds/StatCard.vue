...
<template>
  <div class="ds-stat">
    <div class="ds-stat__top">
      ...
      <span v-if="icon" class="ds-stat__chip" :style="chipStyle">
        <Icon :name="icon" :size="16" />
      </span>
    </div>
    <p class="ds-stat__value">{{ value }}</p>
    <div v-if="raw || hint" class="ds-stat__foot">
      <span v-if="raw" class="ds-stat__delta" :style="deltaStyle">
        <Icon :name="deltaIcon" :size="12" :stroke-width="2.5" />
        {{ raw.replace(/^-/, '') }}
      </span>
      <span v-if="hint">{{ hint }}</span>
    </div>
    <span class="ds-stat__bar" :style="{ background: `var(--ds-tint-${tint}-text)` }" />
  </div>
</template>
