...
// on the kit's own BarChart.jsx reference (ui/docs/DESIGN.md,
// "Provenance"): flex column-per-category (value label + track +
...
<template>
  <div class="ds-chart">
    <div class="ds-bars" role="img" :aria-label="summary">
      <div class="ds-bars__plot" :style="{ height: `${height}px` }">
        <div v-for="b in capped" :key="b.label" class="ds-bars__col">
          <span class="ds-bars__v">{{ format(b.value) }}</span>
          <span class="ds-bars__track">
            <span
              class="ds-bars__bar"
          ...
          <span class="ds-bars__x">{{ b.label }}</span>
        </div>
      </div>
      <div class="ds-bars__scale"><span>0</span><span>{{ format(max) }}</span></div>
    </div>
    <table class="sr-only">
