...
// on the kit's own LineChart.jsx reference (ui/docs/DESIGN.md,
// "Provenance"): gridlines + a visible chart-token stroke + point
...
//
// Fix-round (uiux S2 #2): the previous version defined `.ds-chart__axislabel`
  ...
  <div class="ds-chart">
    ...
    >
      <line
        v-for="i in 3"
        :key="i"
        class="ds-chart__gridline"
        x1="0"
        :x2="width"
        :y1="(height / 4) * i"
        :y2="(height / 4) * i"
      />
      <path v-if="path" :d="path" class="ds-chart__line" vector-effect="non-scaling-stroke" />
      <circle v-for="p in plotted" :key="p.label" :cx="p.x" :cy="p.y" r="3" class="ds-chart__point">
        <title>{{ p.label }}: {{ format(p.value) }}</title>
      </circle>
    </svg>
    <div v-if="xLabels.length > 0" class="ds-chart__axisrow">
      <span v-for="l in xLabels" :key="l.label" class="ds-chart__axislabel">{{ l.label }}</span>
    </div>
    <div v-if="points.length > 0" class="ds-chart__scale">
